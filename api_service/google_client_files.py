'''
    此处重写google.client.files中的Files类, 减少代码侵入和迁移成本
    基于版本google_genai-1.14.0
'''
import io
import mimetypes
import os
...
from google.genai.files import _ListFilesParameters_to_mldev, _ListFilesResponse_from_mldev, \
    _CreateFileParameters_to_mldev, _CreateFileResponse_from_mldev, _GetFileParameters_to_mldev, _File_from_mldev, \
    _DeleteFileParameters_to_mldev, _DeleteFileResponse_from_mldev
...
class Files(_api_module.BaseModule):
    ...
    def upload(
        ...
        # 替换上传地址的域名到代理
        if self._api_client._http_options.base_url:
            upload_url = upload_url.replace("https://generativelanguage.googleapis.com",
                                            self._api_client._http_options.base_url)
